 ...
 *
 * Uses a full RiftProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, RiftProvider>()
  ...
  /** Extract the PanelView from a viewType string like "rift-ai.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^rift-ai\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated RiftProvider for this panel so it has full
    // backend connectivity (config, providers, agents, profile, auth).
    const provider = new RiftProvider(this.extensionUri, this.connectionService, this.context, {
      projectDirectory,
    })
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let RiftProvider's own webviewReady handler finish first
        setTimeout(() => {
