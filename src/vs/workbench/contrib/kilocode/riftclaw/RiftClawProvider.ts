/**
 * RiftClaw panel provider for the VS Code extension.
 *
...
export class RiftClawProvider implements vscode.Disposable {
  static readonly viewType = "rift-ai.new.RiftClawPanel"
  ...
  openPanel(): void {
    ...
    const panel = vscode.window.createWebviewPanel(RiftClawProvider.viewType, "RiftClaw", vscode.ViewColumn.One, {
      enableScripts: true,
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      scriptUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "riftclaw.js")),
      styleUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "riftclaw.css")),
    ...
    const msgSub = panel.webview.onDidReceiveMessage((msg: RiftClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "riftclaw.locale", locale })
    })
  ...
  private post(msg: RiftClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: RiftClawInMessage): Promise<void> {
    switch (msg.type) {
      case "riftclaw.ready":
        await this.init()
        return
      case "riftclaw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("rift-ai.new").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "riftclaw.state", state: { phase: "loading", locale: this.locale } })
      ...
      this.post({ type: "riftclaw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
  ...
  private async bootstrap(client: KiloClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "riftclaw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftClaw chat token fetch failed:", message)
      ...
      this.post({ type: "riftclaw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "riftclaw.error", error: "Authentication expired" })
      },
    })
    this.events = events
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[Kilo New] RiftClaw event-service connect failed:", message)
      this.post({
        type: "riftclaw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Kilo New] RiftClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Kilo New] RiftClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[Kilo New] RiftClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<KiloClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[Kilo New] RiftClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[Kilo New] RiftClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/riftclaw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/riftclaw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient, _chat: KiloChatClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/riftclaw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "riftclaw.activeConversation", conversationId: null })
          this.post({
            type: "riftclaw.messages",
            conversationId: e.conversationId,
            messages: [],
            hasMore: false,
            replace: true,
          })
          this.post({ type: "riftclaw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "riftclaw.botStatus", status: this.botStatus })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "riftclaw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "riftclaw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "riftclaw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "riftclaw.activeConversation", conversationId: null })
        this.post({ type: "riftclaw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "riftclaw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[Kilo New] RiftClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "riftclaw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "riftclaw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Kilo New] RiftClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Kilo New] RiftClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "riftclaw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "riftclaw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      ...
      this.chat.requestBotStatus(this.sandboxId).catch((err) => {
        console.debug("[Kilo New] RiftClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      if (res?.data) {
        ...
        this.post({ type: "riftclaw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[Kilo New] RiftClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
