...
export function registerTerminalActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rift-ai.new.terminalAddToContext", async (args: any) => {
    ...
    vscode.commands.registerCommand("rift-ai.new.terminalFixCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
    ...
    vscode.commands.registerCommand("rift-ai.new.terminalExplainCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
