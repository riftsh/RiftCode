 ...
 * .riftcodeignore / .gitignore files are modified) and shares a single
...
export class ChatTextAreaAutocomplete {
  ...
  constructor(connectionService: KiloConnectionService, telemetry?: AutocompleteTelemetry) {
    ...
    this.watcher = vscode.workspace.createFileSystemWatcher("**/{.riftcodeignore,.gitignore}")
    const invalidate = () => {
  ...
  async getCompletion(userText: string, visibleCodeContext?: VisibleCodeContext): Promise<{ suggestion: string }> {
    const cfg = vscode.workspace.getConfiguration("rift-ai.new.autocomplete")
