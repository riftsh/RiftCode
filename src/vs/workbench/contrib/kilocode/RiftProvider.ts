...
import { saveImage } from "./rift-provider/save-image"
import { exportTranscript } from "./rift-provider/export-transcript"
import {
  TelemetryProxy,
...
import { slimPart, slimParts } from "./rift-provider/slim-metadata"
import { handleSidebarWorktreeMessage } from "./rift-provider/sidebar-worktree"
import { parseMessageFiles, type MessageFile } from "./rift-provider/message-files"
import { renameSession } from "./rift-provider/rename-session"
import { handleFileSearch } from "./rift-provider/file-search"
import { watchFontSizeConfig } from "./rift-provider/font-size"
import { getTerminalContents } from "./services/terminal/context"
import { disposeGitChangesTarget } from "./rift-provider/git-changes-target"
import { interceptMessage } from "./rift-provider/git-changes-request"
import { matchFollowup, recordFollowup, type Followup } from "./rift-provider/followup-session"
import { clearCommandsCache, loadCommands } from "./rift-provider/commands"
import { fetchMessagePage, MESSAGE_PAGE_LIMIT } from "./rift-provider/message-page"
import { childID } from "./rift-provider/task-session"
import { VisibleTaskStreams } from "./rift-provider/visible-task-streams"
import { handleNetworkEvent, clearNetworkWaits } from "./rift-provider/network"
import { abortSession } from "./rift-provider/abort"
import {
  buildAutocompleteSettingsMessage,
  validAutocompleteSetting,
  watchAutocompleteConfig,
} from "./services/autocomplete/settings"
import { routeEarlyMessage } from "./rift-provider/early-message"
import * as ModelState from "./rift-provider/model-state"
import { handleForkSession } from "./rift-provider/fork-session"
import { openConfig } from "./rift-provider/open-config"
import * as McpOAuth from "./rift-provider/mcp-oauth"
import { retryable, backoff, MAX_RETRIES } from "./util/retry"
import { hasGit } from "./rift-provider/git-status"
...
} from "./rift-provider/handlers/migration"
...
import {
  ...
  type AuthContext,
} from "./rift-provider/handlers/auth"
import {
  handleRequestCloudSessions,
  handleRequestCloudSessionData,
  handleImportAndSend,
  type CloudSessionContext,
} from "./rift-provider/handlers/cloud-session"
import {
  handlePermissionResponse,
  fetchAndSendPendingPermissions,
  type PermissionContext,
} from "./rift-provider/handlers/permission-handler"
import {
  handleQuestionReply,
  handleQuestionReject,
  fetchAndSendPendingQuestions,
} from "./rift-provider/handlers/question"
import { fetchAndSendPendingSuggestions } from "./rift-provider/handlers/suggestion"
import { nativeTitle } from "./rift-provider/native-tab-title"
...
import type { Agent } from "@riftcode/sdk/v2/client"
import { configFeatures } from "./features"
import { createAutoApproveBridge } from "./rift-provider/auto-approve"
import type { RiftProviderOptions } from "./rift-provider/options"
import { fetchKiloEmbeddingModelCatalog } from "@riftcode/kilo-gateway"
import { stopSessionProcesses } from "./rift-provider/background-process"
...
export class RiftProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  public static readonly viewType = "rift-ai.SidebarProvider"
  ...
  private readonly extensionVersion =
    vscode.extensions.getExtension("riftcode.rift-ai")?.packageJSON?.version ?? "unknown"
  ...
  constructor(
    ...
    private readonly opts: RiftProviderOptions = {},
  ) {
  ...
  /**
   * Convenience getter that returns the shared SDK RiftClient or null if not yet connected.
   * Preserves the existing null-check pattern used throughout handler methods.
   */
  private get client(): RiftClient | null {
    try {
  ...
  // sessions) to keep session switches fast. Logic in rift-provider/slim-metadata.ts.
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[Kilo New] RiftProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[Kilo New] RiftProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    if (serverInfo) {
      const langConfig = vscode.workspace.getConfiguration("rift-ai.new")
      this.postMessage({
        type: "ready",
        serverInfo,
    ...
    if (this.connectionState === "connected" && this.client) {
      console.log("[Kilo New] RiftProvider: 👤 syncWebviewState fetching profile...")
      const profileResult = await retry(() => this.client!.kilo.profile())
      const profileData = profileResult.data ?? null
      console.log("[Kilo New] RiftProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
      this.postMessage({
        type: "profileData",
        data: profileData,
      })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "rift-ai.new.sidebarVisible", visible)
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("rift-ai.new.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("rift-ai.new.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("rift-ai.new.showChanges", { sessionId, turnId }),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[Kilo New] RiftProvider: ✅ webviewReady received")
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("rift-ai.new.settingsButtonClicked", message.tab)
          break
        ...
        case "openMarketplacePanel":
          vscode.commands.executeCommand("rift-ai.new.marketplaceButtonClicked", this.projectDirectory)
          break
        ...
        case "retryConnection":
          console.log("[Kilo New] RiftProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[Kilo New] RiftProvider: ❌ Retry connection failed:", e),
          )
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand("rift-ai.new.openSubAgentViewer", message.sessionID, message.title)
          break
        case "previewImage":
        ...
        case "openSettingsTab":
          if (message.tab === "indexing") {
            await vscode.commands.executeCommand("rift-ai.new.openIndexingSettings")
          }
          break
        case "setLanguage":
          await vscode.workspace
            .getConfiguration("rift-ai.new")
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[Kilo New] RiftProvider: Failed to enhance prompt:", err)
  ...
  private async doInitializeConnection(): Promise<void> {
    console.log("[Kilo New] RiftProvider: 🔧 Starting initializeConnection...")
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (event) => {
          ...
          // RiftProvider instance. The Settings panel is a separate provider with no tracked
      ...
      this.unsubscribeState = this.connectionService.onStateChange(async (state) => {
        ...
        if (state === "connected") {
          ...
          try {
            ...
            console.error("[Kilo New] RiftProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other RiftProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other RiftProvider instances
      ...
      // Subscribe to profile change broadcast from other RiftProvider instances
      this.unsubscribeProfileChange = this.connectionService.onProfileChanged((data) => {
        this.postMessage({ type: "profileData", data })
      })

      // Subscribe to favorites change broadcast from other RiftProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // legacy-migration start
      // Subscribe to migration-complete broadcast from any RiftProvider instance
      ...
      console.log("[Kilo New] RiftProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[Kilo New] RiftProvider: ❌ Failed to initialize connection:", error)
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private refreshSessionDetails(sessionID: string, dir: string, signal?: AbortSignal): void {
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.warn("[Kilo New] RiftProvider: getSession failed (non-critical):", e))
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.error("[Kilo New] RiftProvider: Failed to fetch session statuses:", e))
  }
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[Kilo New] RiftProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[Kilo New] RiftProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to rename session:", error)
  ...
  private async handleExportSessionTranscript(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to export session transcript:", error)
      this.postMessage({
        type: "error",
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("rift-ai.new.model")
          const message = {
          ...
          console.error("[Kilo New] RiftProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async fetchCliSkills(): Promise<Array<{ name: string; location: string }> | undefined> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch CLI skills for marketplace:", error)
      return undefined
    }
  }
  ...
  private async removeSkillViaCli(location: string): Promise<boolean> {
    ...
    try {
      const dir = this.getWorkspaceDirectory()
      const result = await this.client.riftcode.removeSkill({ location, directory: dir })
      if (result.error) {
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    if (!this.client) return
    try {
      const result = await this.client.riftcode.removeAgent({ name, directory: this.getWorkspaceDirectory() })
      if (!result.error) {
    ...
    if (!(await this.removeMarketplaceItemFromAllScopes(stub))) {
      console.error("[Kilo New] RiftProvider: Failed to remove agent:", name)
    }
  }
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[Kilo New] RiftProvider: Failed to remove MCP server:", name)
    }
  }

  /**
   * Remove an MCP server from legacy config files (.kilo/mcp.json, .riftcode/mcp.json,
  ...
  private async removeLegacyMcp(name: string): Promise<boolean> {
    ...
    if (workspace) {
      ...
      files.push(vscode.Uri.file(path.join(workspace, ".riftcode", "mcp.json")))
    }
    ...
    for (const uri of files) {
      ...
      try {
        ...
        console.warn("[Kilo New] RiftProvider: Failed to remove legacy MCP from", uri.fsPath, err)
      }
    }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch global config:", error)
    }
  }
  ...
  private async fetchAndSendIndexingStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch indexing status:", error)
    }
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[Kilo New] RiftProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[Kilo New] RiftProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[Kilo New] RiftProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[Kilo New] RiftProvider: config warnings fetched", { from, count: list.length })
      if (list.length === 0) return
      this.configWarningsShown = true

      const first = list[0]!
      const summary = list.length === 1 ? first.message : `${first.message} (and ${list.length - 1} more)`
      console.warn("[Kilo New] RiftProvider: showing config warnings", { from, count: list.length, path: first.path })
      ...
      console.warn("[Kilo New] RiftProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  private async fetchAndSendNotifications(): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to fetch notifications:", error)
    }
  }

  // Cloud session methods extracted to rift-provider/handlers/cloud-session.ts
  ...
  private sendNotificationSettings(): void {
    const notifications = vscode.workspace.getConfiguration("rift-ai.new.notifications")
    const sounds = vscode.workspace.getConfiguration("rift-ai.new.sounds")
    this.postMessage({
  ...
  private sendTimelineSetting(): void {
    const config = vscode.workspace.getConfiguration("rift-ai.new")
    this.postMessage({
  ...
  private async handleUpdateConfig(partial: Partial<Config>, project: Partial<Config> = {}): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Config write succeeded but post-write refresh failed:", error)
      const patch =
  ...
  private postConfigFailure(error: unknown): void {
    console.error("[Kilo New] RiftProvider: Failed to update config:", error)
    this.postMessage({
  ...
  private async withRetry(
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        console.log(`[Kilo New] RiftProvider: Retry on ${status}, attempt ${attempt}/${MAX_RETRIES}, delay ${delay}ms`)
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "RiftProvider: Message request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Kilo New] RiftProvider: Failed to send message:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      const sid = resolved!.sid
      const dir = resolved!.dir
      await runWithMessageConfirmation(this.confirmations, messageID, "RiftProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Kilo New] RiftProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleAbort(sessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to abort session:", error)
    }
  }
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[Kilo New] RiftProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[Kilo New] RiftProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[Kilo New] RiftProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[Kilo New] RiftProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  // Permission + question handlers extracted to rift-provider/handlers/permission.ts and question.ts
  ...
  // Cloud session handlers extracted to rift-provider/handlers/cloud-session.ts
  ...
  // Auth handlers extracted to rift-provider/handlers/auth.ts
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await this.client.global
      .dispose()
      .catch((e: unknown) => console.warn("[Kilo New] RiftProvider: global.dispose() after org switch failed:", e))
    ...
    try {
      ...
      console.error("[Kilo New] RiftProvider: Failed to refresh profile after org switch:", error)
    }
    try {
      await this.fetchAndSendProviders()
    } catch (error) {
      console.error("[Kilo New] RiftProvider: Failed to refresh providers after org switch:", error)
    }
  }
  ...
  private handlePreviewImage(dataUrl: string, filename: string): void {
          ...
          return Promise.all(
            stale.map((name) =>
              Promise.resolve(vscode.workspace.fs.delete(vscode.Uri.joinPath(root, name), { recursive: true })).then(
                undefined,
                (err: unknown) => {
                  console.warn("[Kilo New] RiftProvider: Failed to delete stale preview:", err)
                },
              ),
            ),
          )
    ...
    void vscode.workspace.fs
      ...
      .then(() => clean())
      .then(open, (err) => console.error("[Kilo New] RiftProvider: Failed to preview image:", err))
  }
  ...
  private handleOpenContent(content: string, language?: string): void {
    vscode.workspace.openTextDocument({ content, language: language || "log" }).then(
      ...
      (err) => console.error("[Kilo New] RiftProvider: Failed to open content:", err),
    )
  }
    ...
    vscode.workspace.fs.stat(uri).then(
      (stat) => {
        ...
        vscode.workspace.openTextDocument(uri).then(
          ...
          (err) => console.error("[Kilo New] RiftProvider: Failed to open file:", uri.fsPath, err),
        )
      },
      (err) => console.error("[Kilo New] RiftProvider: Path does not exist:", uri.fsPath, err),
    )
  }

  /**
   * Handle a generic setting update from the webview.
   * The key uses dot notation relative to `rift-ai.new` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown): Promise<void> {
    const { section, leaf } = buildSettingPath(key)
    if (section === "autocomplete" && !validAutocompleteSetting(leaf, value)) return
    const config = vscode.workspace.getConfiguration(`rift-ai.new${section ? `.${section}` : ""}`)
  ...
  /**
   * Reset all "rift-ai.new.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "rift-ai.new." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "rift-ai.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all RiftAI extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "rift-ai.new."
    const ext = vscode.extensions.getExtension("riftcode.rift-ai")
    ...
    vscode.window.showInformationMessage("RiftAI settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("rift-ai.new.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("rift-ai.new").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private handleEvent(event: Event, directory?: string): void {
    ...
    // so this runs on every RiftProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[Kilo New] RiftProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[Kilo New] RiftProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[Kilo New] RiftProvider: ❌ postMessage failed", error)
    })
  }
  ...
  public async appendReviewComments(comments: unknown[], autoSend = false): Promise<void> {
    ...
    if (!this.webview) {
      await vscode.commands.executeCommand(`${RiftProvider.viewType}.focus`)
    }
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[Kilo New] RiftProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
   ...
   * given directory, filtered through .riftcodeignore.
   */
  ...
  private async gatherEditorContext(dir?: string): Promise<EditorContext> {
    ...
    // Visible files (capped to avoid bloating context, filtered through .riftcodeignore)
    ...
    // Active file (also filtered through .riftcodeignore)
  ...
  // legacy-migration start -------------------------------------------------------
  // Migration handlers extracted to rift-provider/handlers/migration.ts
