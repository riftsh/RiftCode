...
import type { KiloClient, Session } from "@riftcode/sdk/v2/client"
import type { KiloConnectionService } from "../services/cli-backend"
import { getErrorMessage } from "../rift-provider-utils"
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "rift-ai.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "riftcode.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .riftcode → .kilo migration rewrote git worktree refs, nudge
   ...
   * Called from RiftProvider when the sidebar sends "continueInWorktree".
   */
