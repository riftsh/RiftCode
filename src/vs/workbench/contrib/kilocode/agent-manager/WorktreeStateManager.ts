...
export class WorktreeStateManager {
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .riftcode → .kilo before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .riftcode paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.riftcode([/\\])/g, (_match, leadingSep, trailingSep) => {
