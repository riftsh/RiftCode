...
describe("WorktreeStateManager", () => {
  ...
  describe("legacy .riftcode migration", () => {
    it("migrates and loads state from .riftcode when .kilo is absent", async () => {
      // Remove the .kilo dir created in beforeEach
      fs.rmSync(path.join(root, ".kilo"), { recursive: true, force: true })

      // Write state to legacy .riftcode dir (migration will move it to .kilo)
      const legacyDir = path.join(root, ".riftcode")
    ...
    it("rewrites stale .riftcode paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".kilo", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.riftcode/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
    ...
    it("rewrites stale .riftcode paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".kilo", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
