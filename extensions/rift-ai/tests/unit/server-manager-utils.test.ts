...
describe("cli tree-sitter resources", () => {
  it("resolves resources next to the VS Code bundled CLI", () => {
    const root = "/Users/test/.vscode/extensions/riftcode.rift-ai-7.2.50-darwin-arm64"
  ...
  it("resolves resources next to a Windows packaged CLI", () => {
    const root = String.raw`C:\Users\test\.vscode\extensions\riftcode.rift-ai-7.2.50-win32-x64`
  ...
  it("copies runtime and language WASMs with the packaged CLI binary", async () => {
    ...
    try {
      const source = path.join(root, "dist", "@riftcode", "cli-darwin-arm64", "bin", "kilo")
