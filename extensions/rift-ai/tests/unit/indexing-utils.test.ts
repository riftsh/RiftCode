...
import type { EventIndexingStatus, IndexingStatus } from "@riftcode/sdk/v2/client"
...
describe("indexing feature detection", () => {
  ...
  it("detects supported indexing plugin specifiers", () => {
    ...
    expect(configFeatures({ plugin: ["@riftcode/kilo-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@riftcode/kilo-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/.opencode/plugin/kilo-indexing.js"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/node_modules/@riftcode/kilo-indexing/index.js"] }).indexing).toBe(
      true,
    )
  })

  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@riftcode/kilo-gateway"] }).indexing).toBe(false)
