...
describe("t()", () => {
  it("returns translated string for known key", () => {
    const result = t("riftcode:autocomplete.statusBar.enabled")
    expect(typeof result).toBe("string")
    expect(result.length).toBeGreaterThan(0)
    expect(result).not.toBe("riftcode:autocomplete.statusBar.enabled")
  })
  ...
  it("interpolates a single variable", () => {
    const result = t("riftcode:autocomplete.statusBar.tooltip.noUsableProvider", {
      providers: "OpenAI, Anthropic",
      command: "command:rift-ai.new.settingsButtonClicked",
    })
  ...
  it("interpolates multiple variables", () => {
    const result = t("riftcode:autocomplete.statusBar.tooltip.completionSummary", {
      count: "5",
      startTime: "10:00",
      endTime: "11:00",
      cost: "$0.05",
  ...
  it("leaves unreferenced vars intact in template", () => {
    const key = "riftcode:autocomplete.statusBar.tooltip.noUsableProvider"
