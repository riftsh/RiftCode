...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "rift-ai.new.agentManager.jumpTo": "registerCommand(`rift-ai.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the rift-ai.new. prefix.
   * The legacy rift-ai.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the rift-ai.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("rift-ai.new."))
    expect(
      bad,
      `Commands without "rift-ai.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
})
...
describe("Extension — KiloProvider handler wiring", () => {
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"rift-ai.new.TabPanel"')
