...
const { RiftCodeActionProvider } = await import("../../src/services/code-actions/code-action-provider")

const provider = new RiftCodeActionProvider()
...
describe("RiftCodeActionProvider", () => {
  describe("provideCodeActions", () => {
    ...
    describe("non-empty range, no diagnostics", () => {
      it("returns Add, Explain, Improve actions", () => {
        ...
        expect(titles).toContain("Add to RiftAI")
        expect(titles).toContain("Explain with RiftAI")
        expect(titles).toContain("Improve with RiftAI")
      })

      it("does not include Fix action", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(0) as never)
        expect(result.map((a) => a.title)).not.toContain("Fix with RiftAI")
      })
      ...
      it("uses correct command IDs", () => {
        ...
        expect(commands).toContain("rift-ai.new.addToContext")
        expect(commands).toContain("rift-ai.new.explainCode")
        expect(commands).toContain("rift-ai.new.improveCode")
      })
    ...
    describe("non-empty range, with diagnostics", () => {
      it("returns Add and Fix actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).toContain("Add to RiftAI")
        expect(titles).toContain("Fix with RiftAI")
      })
      ...
      it("does not include Explain or Improve actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).not.toContain("Explain with RiftAI")
        expect(titles).not.toContain("Improve with RiftAI")
      })
      ...
      it("Fix action is preferred", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with RiftAI")
      ...
      it("uses correct Fix command ID", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with RiftAI")
        expect(fix?.command?.command).toBe("rift-ai.new.fixCode")
      })
    })
  })
})
