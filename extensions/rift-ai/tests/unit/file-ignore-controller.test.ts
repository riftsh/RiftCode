...
describe("FileIgnoreController", () => {
  describe("when .riftcodeignore exists", () => {
    it("applies only .riftcodeignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".riftcodeignore"), "secret/**\n*.snap\n")
      ...
      expect(controller.getInstructions()).toContain(".riftcodeignore")
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".riftcodeignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .riftcodeignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .riftcodeignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .riftcodeignore, no .gitignore
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross =
        "c:/Users/User/AppData/Roaming/Code/User/globalStorage/riftcode.rift-ai/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/riftcode.rift-ai/settings/mcp_settings.json"
