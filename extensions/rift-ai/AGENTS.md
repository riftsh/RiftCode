...
RiftAI is an open source AI coding agent platform. It ships as a CLI and editor clients that all build on the same backend. This package (`packages/kilo-vscode/`) is the **VS Code extension**.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@riftcode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Kilo-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `kilo serve` process and communicates via HTTP REST + SSE using the auto-generated `@riftcode/sdk`.

```
                        @riftcode/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
| Package | Name | Role |
|---|---|---|
| `packages/kilo-vscode/` | `rift-ai` | **This package.** VS Code extension. |
| `packages/kilo-gateway/` | `@riftcode/kilo-gateway` | Auth (device flow), AI provider routing (OpenRouter), Kilo API integration (profile, balance, teams) |
| `packages/kilo-ui/` | `@riftcode/kilo-ui` | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and docs screenshot stories |
| `packages/kilo-telemetry/` | `@riftcode/kilo-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI |
| `packages/kilo-i18n/` | `@riftcode/kilo-i18n` | Translation strings (16 languages) |
| `packages/kilo-docs/` | `@riftcode/kilo-docs` | Documentation site (Next.js + Markdoc) |

### Upstream OpenCode Packages (not Kilo-specific)

| Package | Name | Role |
|---|---|---|
| `packages/opencode/` | `@riftcode/cli` | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server. |
| `packages/sdk/js/` | `@riftcode/sdk` | Auto-generated TypeScript SDK client for the server API. Do not edit `src/gen/` by hand. |
| `packages/ui/` | `@opencode-ai/ui` | Shared UI primitives |
| `packages/util/` | `@opencode-ai/util` | Shared utilities (error, path, retry, slug) |
| `packages/plugin/` | `@riftcode/plugin` | Plugin/tool interface definitions |
...
New webview features must use **`@riftcode/kilo-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@riftcode/kilo-ui/button"`
...
- Global styles imported via `import "@riftcode/kilo-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
...
- All VSCode commands must use `rift-ai.new.` prefix (not `rift-ai.`)
- All view IDs must use `rift-ai.new.` prefix, **except** the sidebar view which uses `rift-ai.SidebarProvider` to preserve user sidebar position when upgrading from the legacy extension

## Kilocode Change Markers

This package is entirely Kilo-specific — `riftcode_change` markers are NOT needed in any files under `packages/kilo-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
