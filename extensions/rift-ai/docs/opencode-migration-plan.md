# Feature Parity Plan — RiftAI VS Code Extension (Rebuild)

## Overview

This extension is a **ground-up rebuild** of the [old RiftAI extension](https://github.com/Kilo-Org/riftcode-legacy) using Kilo CLI as the backend. Rather than migrating the old extension's codebase, we started fresh with a Solid.js webview, a CLI server manager, and a message-based protocol between extension host and webview. This new extension lives in the [riftcode monorepo](https://github.com/Kilo-Org/riftcode/tree/main/packages/kilo-vscode).
...
| [Dedicated Output Channel](infrastructure/dedicated-output-channel.md) | General "RiftAI" output channel and centralized logging utility | P2 |
...
- [ ] **Dedicated output channel** — All logging currently goes to `console.log` mixed with other extensions ([details](infrastructure/dedicated-output-channel.md)). Create a dedicated "RiftAI" output channel before production
...
- **kilo-ui shared library**: The webview now heavily uses `@riftcode/kilo-ui` for UI components. A `DataBridge` component in App.tsx adapts the session store to kilo-ui's `DataProvider` expected shape, enabling shared components like `<KiloMessage>` to work with the extension's data model.
