...
1. **`bun run watch`** is already running for the riftcode extension.
2. In the riftcode VSCode window, press **F5** → opens the **Extension Development Host**.
3. In the Dev Host: `File → Open Folder…` → `packages/kilo-vscode/docs/nes-examples/` inside this repo.
4. Open Settings (`Cmd+,`), confirm:
   - `rift-ai.new.autocomplete.enableAutoTrigger` → ✓ (default true)
   - `rift-ai.new.autocomplete.model` → **Mercury Next Edit (Inception)** ← *NOT* "Mercury Edit 2", which is the classic FIM option
   - `rift-ai.new.autocomplete.nextEdit.apiKey` → your `sk_...` key
   - VSCode global `editor.inlineSuggest.enabled` → ✓
5. To watch the pipeline live: `View → Output` → pick the **"RiftAI · Next Edit"** channel.
...
- **No log lines appearing**: confirm the output channel is "RiftAI · Next Edit". Also confirm you reloaded the Dev Host after rebuilding.
