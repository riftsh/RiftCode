...
| Rule source discovery | Auto-discovers from `.riftcode/rules/`, `.riftcoderules`, `.roorules`, `.clinerules`, mode-specific dirs, `AGENTS.md` | Only shows explicitly configured paths in `config.instructions` |
...
- **Auto-discovery**: Consider adding a section showing auto-discovered rule sources (`.riftcode/rules/`, `AGENTS.md`) that the CLI loads regardless of explicit `config.instructions` entries
