...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.riftcode/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.riftcode/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.riftcode/rules/rules.md` | Not available |
