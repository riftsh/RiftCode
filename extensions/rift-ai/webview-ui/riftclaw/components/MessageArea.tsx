// RiftClaw active-conversation view — message list + composer.
...
import { createAutoScroll } from "@riftcode/kilo-ui/hooks"
...
export function MessageArea() {
  ...
  //
  // Uses @riftcode/kilo-ui's createAutoScroll (the same one MessageList.tsx
  ...
  // `bot:riftclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "RiftClaw" label) and render any human
  ...
  return (
    <Show
      ...
      fallback={
        <div class="riftclaw-empty-area">
          <p>{t("kiloClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="riftclaw-area">
        {/* Messages */}
        <div class="riftclaw-area-messages-wrap">
          <div
            class="riftclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="riftclaw-reply-preview">
              <span class="riftclaw-reply-preview-label">{t("kiloClaw.message.replyTo")}</span>
              <span class="riftclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
        ...
        <div class="riftclaw-input-wrap">
          <textarea
            ref={input}
            class="riftclaw-input"
