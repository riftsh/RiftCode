// RiftClaw conversation sidebar — mirrors the web UI in
...
export function ConversationList() {
  ...
  return (
    <div class="riftclaw-convlist">
      <div class="riftclaw-convlist-header">
        <span class="riftclaw-convlist-title">{t("kiloClaw.conversations.title")}</span>
        <button
          type="button"
          class="riftclaw-iconbtn"
      ...
      <div class="riftclaw-convlist-scroll" ref={scrollEl}>
        <Show
          when={claw.conversations().length > 0}
          fallback={<div class="riftclaw-convlist-empty">{t("kiloClaw.conversations.empty")}</div>}
        >
          <For each={groups()}>
            {(group) => (
              <div class="riftclaw-convlist-group">
                <div class="riftclaw-convlist-grouplabel">{group.label}</div>
...
function ConversationItem(props: { conversation: ConversationListItem }) {
  ...
  return (
    <div
      class={`riftclaw-convitem ${isActive() ? "riftclaw-convitem-active" : ""}`}
      onClick={() => {
    ...
    >
      <Show
        ...
        fallback={
          <input
            ref={inputEl}
            class="riftclaw-convitem-renameinput"
            value={renameText()}
      ...
      >
        <span class="riftclaw-convitem-title">
          <Show when={isUnread()}>
            <span class="riftclaw-convitem-unread" aria-hidden="true" />
          </Show>
          {props.conversation.title ?? t("kiloClaw.conversations.untitled")}
        </span>
      </Show>
      <div class="riftclaw-convitem-actions" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
