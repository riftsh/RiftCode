...
import { Markdown } from "@riftcode/kilo-ui/markdown"
...
export function MessageBubble(props: MessageBubbleProps) {
  ...
  return (
    <div class={`riftclaw-msg riftclaw-msg-${variant()}`}>
      <div class="riftclaw-msg-column">
        {/* Author label — above the bubble for bot messages only */}
        <Show when={showAuthor()}>
          <span class="riftclaw-msg-author">{props.assistantName ?? t("kiloClaw.message.bot")}</span>
        </Show>

        {/* Reply preview — above the bubble, aligned with the author label */}
        <Show when={props.replyToMessage}>
          {(reply) => (
            <div class="riftclaw-msg-reply">
              <span class="riftclaw-msg-reply-arrow" aria-hidden="true">
                ↩
              </span>
              <Show
                when={!reply().deleted}
                fallback={<span class="riftclaw-msg-reply-deleted">{t("kiloClaw.message.replyDeleted")}</span>}
              >
                <span class="riftclaw-msg-reply-text">
                  {(() => {
        ...
        <div class="riftclaw-msg-bubble-wrap">
          <Show when={!props.message.deleted && !isEditing() && !isDeleting() && !isOptimistic()}>
            <div class="riftclaw-msg-toolbar">
              <button
                type="button"
                class="riftclaw-iconbtn-sm"
              ...
              <Show when={props.isOwn}>
                <button
                  type="button"
                  class="riftclaw-iconbtn-sm riftclaw-iconbtn-danger"
          ...
          <Show when={showReactionPick()}>
            <div class="riftclaw-msg-reactionpick">
              <For each={["👍", "❤️", "😂", "🎉", "🚀", "👀"]}>
                {(emoji) => (
                  <button
                    type="button"
                    class="riftclaw-msg-reactionpick-btn"
                    onClick={() => {
          ...
          <div class="riftclaw-msg-bubble">
            <Show
              when={!props.message.deleted}
              fallback={<span class="riftclaw-msg-deleted">{t("kiloClaw.message.deleted")}</span>}
            >
              <Show when={isEditing()}>
                <div class="riftclaw-msg-edit">
                  <textarea
                    class="riftclaw-msg-edit-input"
                    value={editText()}
                    ...
                    autofocus
                  />
                  <div class="riftclaw-msg-edit-actions">
                    <button
                      type="button"
                      class="riftclaw-iconbtn-sm"
                      onClick={saveEdit}
              ...
              <Show when={isDeleting()}>
                <div class="riftclaw-msg-confirm-delete">
              ...
              <Show when={!isEditing() && !isDeleting()}>
                <div class="riftclaw-msg-body">
                  <Show
                    when={!empty()}
                    fallback={<span class="riftclaw-msg-thinking">{t("kiloClaw.message.thinking")}</span>}
                  >
                    <Show when={isBot()} fallback={<span class="riftclaw-msg-text">{textContent()}</span>}>
                ...
                {(block) => {
                  ...
                  return (
                    <Show
                      when={!block.resolved}
                      fallback={
                        <div class="riftclaw-msg-actions-resolved">
                          <span class="riftclaw-msg-actions-resolved-icon">
                    ...
                    >
                      <div class="riftclaw-msg-actions">
                        <For each={block.actions}>
                          {(action) => (
                            <button
                              type="button"
                              class={`riftclaw-msg-action riftclaw-msg-action-${action.style}`}
            ...
            <Show when={!isEditing() && !isDeleting()}>
              <div class="riftclaw-msg-footer">
                <Show when={props.message.deliveryFailed}>
                  <span class="riftclaw-msg-failed">{t("kiloClaw.message.notDelivered")}</span>
                </Show>
          ...
          <Show when={!props.message.deleted && props.message.reactions.length > 0}>
            <div class="riftclaw-msg-reactions">
              <For each={props.message.reactions}>
                {(r) => (
                  <button
                    type="button"
                    class="riftclaw-msg-reaction-pill"
