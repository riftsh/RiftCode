/**
 * Provider action handlers extracted from RiftProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, KiloClient } from "@riftcode/sdk/v2"
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Kilo New] RiftProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
