/**
 * legacy-migration - Types for legacy RiftAI extension (v5.x) data structures.
 *
 * These types represent the shapes stored in VS Code SecretStorage and on disk
 * by the legacy extension (riftcode.rift-ai v5.x, a Roo Code fork).
...
export interface LegacyProviderSettings {
  ...
  // Kilocode
  riftcodeToken?: string
  riftcodeModel?: string
  riftcodeOrganizationId?: string
...
// ---------------------------------------------------------------------------
// Settings (stored in VS Code globalState under "rift-ai.*" keys)
// ---------------------------------------------------------------------------
