...
export async function migrate(
  ...
  try {
    ...
    const project = await client.riftcode.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.riftcode.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.riftcode.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.riftcode.sessionImport.part(part, { throwOnError: true })
    }
