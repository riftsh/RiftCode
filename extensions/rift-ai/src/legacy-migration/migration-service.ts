 ...
 *
 * Reads legacy RiftAI v5.x data from VS Code SecretStorage and the extension's
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "rift-ai.allowedCommands",
    "rift-ai.deniedCommands",
    "rift-ai.autoApprovalEnabled",
    "rift-ai.fuzzyMatchThreshold",
    "rift-ai.diffEnabled",
    "rift-ai.language",
    "rift-ai.customModes",
    "rift-ai.firstInstallCompleted",
    "rift-ai.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "rift-ai" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("rift-ai")
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("rift-ai.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("rift-ai.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("rift-ai.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("rift-ai.deniedCommands"),
    ...
    language: context.globalState.get<string>("rift-ai.language"),
