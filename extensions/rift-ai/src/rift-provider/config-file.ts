...
const HOME = [".kilo", ".riftcode", ".opencode"]
const SOURCES: Record<string, Source> = {
  ".kilo": "sourceHomeKilo",
  ".riftcode": "sourceHomeKilocode",
...
function row(file: string, source: Source, loaded = true, recommended = false): Entry {
  ...
  return {
    ...
    legacy: name.startsWith("opencode") || name === "config.json" || file.includes(`${path.sep}.riftcode${path.sep}`),
    recommended,
  }
}
...
export function localFiles(root: string) {
  ...
  const dirs = [path.join(root, ".kilo"), root, path.join(root, ".riftcode"), path.join(root, ".opencode")]
...
function localSource(root: string, dir: string) {
  ...
  if (dir.endsWith(`${path.sep}.riftcode`)) return "sourceProjectKilocode"
