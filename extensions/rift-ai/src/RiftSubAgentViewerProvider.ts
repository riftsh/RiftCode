 ...
 *
 * Uses a full RiftProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, RiftProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("rift-ai.new.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
