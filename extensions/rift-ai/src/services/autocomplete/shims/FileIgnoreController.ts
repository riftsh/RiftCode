...
const KILOCODEIGNORE = ".riftcodeignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .riftcodeignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .riftcodeignore first — if it exists, use only that.
    ...
    const riftcodeignorePath = path.join(this.workspacePath, KILOCODEIGNORE)
    if (fs.existsSync(riftcodeignorePath)) {
      const riftcodeignoreContent = fs.readFileSync(riftcodeignorePath, "utf-8")
      if (riftcodeignoreContent.trim()) {
        this.ignoreInstance.add(riftcodeignoreContent)
        this.ignoreInstance.add(KILOCODEIGNORE)
        this.loadedContents.push({ file: KILOCODEIGNORE, content: riftcodeignoreContent })
        return
      }
    }
