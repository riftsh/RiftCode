...
const CONFIG_SECTION = "rift-ai.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "riftcode.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    const msg =
      status === 402
        ? t("riftcode:autocomplete.creditsExhausted.message")
        : t("riftcode:autocomplete.authError.message")

    if (status === 402) {
      vscode.window.showWarningMessage(msg, t("riftcode:autocomplete.creditsExhausted.addCredits")).then((choice) => {
        if (choice === t("riftcode:autocomplete.creditsExhausted.addCredits")) {
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("riftcode:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("riftcode:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("riftcode:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("rift-ai.new.autocomplete.disable")
    }
  }
