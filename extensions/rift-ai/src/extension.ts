...
import { RiftProvider } from "./RiftProvider"
import { AgentManagerProvider } from "./agent-manager/AgentManagerProvider"
import { VscodeHost } from "./agent-manager/vscode-host"
import { RiftClawProvider } from "./riftclaw/RiftClawProvider"
...
import { registerCodeActions, registerTerminalActions, RiftCodeActionProvider } from "./services/code-actions"
...
export function activate(context: vscode.ExtensionContext) {
  console.log("RiftAI extension is now active")
  shuttingDown = false
  ...
  // Create RiftBrowser service (built-in Chromium browser panel)
  const riftBrowserService = new RiftBrowserService(context.extensionUri)
  context.subscriptions.push(riftBrowserService)
  ...
  const tabPanels = new Map<vscode.WebviewPanel, RiftProvider>()
  ...
  const provider = new RiftProvider(context.extensionUri, connectionService, context, {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(RiftProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["rift-ai.new.agentManagerOpen", "rift-ai.new.agentManager.showTerminal"]
  if (process.platform === "darwin") skip.push("rift-ai.new.agentManager.runScript")
  ensureCommandsSkipShell(skip)

  // Create RiftClaw chat provider for editor panel
  const kiloClawProvider = new RiftClawProvider(context.extensionUri, connectionService)
  ...
  // Register serializer so RiftClaw panel restores when VS Code restarts
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(RiftClawProvider.viewType, {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("rift-ai.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new RiftProvider(context.extensionUri, connectionService, context, {
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`rift-ai.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("rift-ai.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "rift-ai.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "rift-ai.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "rift-ai.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.kiloClawOpen", () => {
      track("riftclaw", "rift-ai.new.kiloClawOpen")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "rift-ai.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "rift-ai.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("rift-ai.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "rift-ai.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("rift-ai.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("rift-ai.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("rift-ai.new.marketplaceButtonClicked", (directory?: string) => {
      settingsEditorProvider.openPanel("marketplace", undefined, directory)
    }),
    vscode.commands.registerCommand("rift-ai.new.kiloClawOpen", () => {
      kiloClawProvider.openPanel()
    }),
    vscode.commands.registerCommand("rift-ai.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("rift-ai.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("rift-ai.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("rift-ai.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("rift-ai.new.settingsButtonClicked", (tab?: string) => {
      settingsEditorProvider.openPanel("settings", tab)
    }),
    vscode.commands.registerCommand("rift-ai.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    // legacy-migration start
    vscode.commands.registerCommand("rift-ai.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("rift-ai.new.generateTerminalCommand", async () => {
      ...
      if (!input) return
      await vscode.commands.executeCommand("rift-ai.SidebarProvider.focus")
      await provider.waitForReady()
      provider.postMessage({ type: "triggerTask", text: `Generate a terminal command: ${input}` })
    }),
    vscode.commands.registerCommand("rift-ai.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Kilo New] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("rift-ai.new.openInTab", () => {
    ...
    vscode.commands.registerCommand(
      "rift-ai.new.showChanges",
      (arg?: { sessionId?: string; turnId?: string; initialSourceId?: string }) => {
        diffViewerProvider.openFromCommand(arg)
      },
    ),
    vscode.commands.registerCommand("rift-ai.new.openSubAgentViewer", (sessionID: string, title?: string) => {
      subAgentViewerProvider.openPanel(sessionID, title)
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("rift-ai.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("rift-ai.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.newTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminal" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("rift-ai.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`rift-ai.new.agentManager.jumpTo${i + 1}`, () => {
        agentManagerProvider.postMessage({ type: "action", action: `jumpTo${i + 1}` })
      }),
    ),
  )

  // Register URI handler for session imports (vscode://riftcode.rift-ai/riftcode/s/{sessionId})
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const match = uri.path.match(/^\/riftcode\/s\/([a-zA-Z0-9_-]+)$/)
        if (!match) return
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      { scheme: "file" },
      new RiftCodeActionProvider(),
      RiftCodeActionProvider.metadata,
    ),
  )
...
async function openKiloInNewTab(
  ...
  tabPanels: Map<vscode.WebviewPanel, RiftProvider>,
  ...
  const panel = vscode.window.createWebviewPanel("rift-ai.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "rift-ai-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "rift-ai-dark.svg"),
  }
